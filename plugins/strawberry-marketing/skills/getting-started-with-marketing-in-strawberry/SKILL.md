...
result. When the voice is not already understood, read `strawberry/learn-writing-style` to learn and
...
themes. Use `strawberry/extract-web-data` to collect approved public evidence into a consistent
...
understands the user's own offer. Read `strawberry/run-competitor-analysis` for the detailed
...
signal. Read `strawberry/create-beautiful-slide-deck` when the result should be a visual artifact,
or `strawberry/research-companies-create-decks` when research and presentation belong in the same
...
actual recent content rather than follower count alone. Use `strawberry/extract-web-data` to create
the reviewable research dataset, then `strawberry/personalized-outreach-sequence` when verified work
...
and which experiment is worth running next. Read `strawberry/audit-seo-create-report` for a
...
`strawberry/set-up-shared-team-workflow` and pilot the accepted process with them. Add monitoring or
