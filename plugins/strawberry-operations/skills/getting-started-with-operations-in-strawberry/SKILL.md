...
preparation, read `strawberry/prepare-for-calls`.
...
`strawberry/build-client-progress-report-project-tools`.
...
style. Read `strawberry/reach-inbox-zero` for inbox organization and reviewable drafts, or
`strawberry/automate-customer-support` when the repeated queue is customer support. Start with a
...
questions. Read `strawberry/do-due-diligence-vendor-or-supplier` for vendor work. When a decision
needs repeated fields from websites, agree on the schema and scope, then read
`strawberry/extract-web-data` to create a reviewable dataset.
...
owned actions, and a follow-up draft. Read `strawberry/transcribe-meetings-follow-up`; nothing sends
or writes to an external system without approval.

When an accepted individual process should become shared, read
`strawberry/set-up-shared-team-workflow` to make owners, destinations, field meanings, privacy
boundaries, and approval behavior explicit, then verify one real handoff before scaling. For
joining, changing roles, or leaving the team, read `strawberry/onboard-offboard-teammates`.
