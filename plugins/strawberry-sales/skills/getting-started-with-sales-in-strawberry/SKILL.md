...
`strawberry/source-customers` for the sourcing workflow.
...
`strawberry/enrich-lead-list` when the starting point is an existing list rather than a new search.
...
`strawberry/research-prospect-before-meeting` for a concise account brief, or
`strawberry/research-companies-create-decks` when the result should be a sourced visual deck.
...
`strawberry/personalized-outreach-sequence` for the detailed workflow; nothing is sent or enrolled
...
`strawberry/prepare-for-calls` before meetings. Afterwards,
`strawberry/transcribe-meetings-follow-up` can turn a transcript in the meeting's language into
...
without silently changing the pipeline. Read `strawberry/keep-crm-updated` for that handoff.

When an individual process works reliably and sharing it would solve a real coordination problem,
read `strawberry/set-up-shared-team-workflow` to establish common fields, owners, destinations,
