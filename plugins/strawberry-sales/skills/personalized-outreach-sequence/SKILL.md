...
- the intended writing style; if it is not already clear, use `strawberry/learn-writing-style` to
...
- Use `strawberry/keep-crm-updated` to preserve approved sales activity and follow-up.
- Use `strawberry/prepare-for-calls` when the next step is a scheduled conversation.
