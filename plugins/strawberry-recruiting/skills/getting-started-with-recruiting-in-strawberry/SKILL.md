...
aside and why so the user can calibrate the filtering. Read `strawberry/source-candidates` for the
...
messages without approval. Read `strawberry/screen-applicants-against-job-description` for the
...
When the voice is not already clear, read `strawberry/learn-writing-style` to learn it from
approved sent messages, user-selected examples, or both. Reuse the evidence and calibration from
`strawberry/source-candidates`, personalize the message body with a specific verified detail, and
...
`strawberry/personalized-outreach-sequence` for the detailed drafting, review, sending, and
follow-up workflow.
...
concise brief. Read `strawberry/prepare-for-calls` for the preparation workflow.
...
and prepare approved ATS updates. Read `strawberry/transcribe-meetings-follow-up` for the
...
`strawberry/set-up-shared-team-workflow` to establish common owners, field meanings, destinations,
