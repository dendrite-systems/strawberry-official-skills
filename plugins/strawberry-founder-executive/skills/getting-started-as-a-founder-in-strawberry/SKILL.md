...
When one conversation matters most, read `strawberry/prepare-for-calls`. It turns relevant
calendar, company, person, and relationship context into a brief the founder can scan before the
meeting.

After the meeting, read `strawberry/transcribe-meetings-follow-up` to turn the transcript into
...
If that first run reveals a larger communication backlog, read `strawberry/reach-inbox-zero`. It
...
structure. Read `strawberry/create-beautiful-slide-deck` when the result should be a polished
...
source types, depth, and desired output. Read `strawberry/research-companies-create-decks` for a
sourced visual brief, or `strawberry/run-competitor-analysis` for a broader comparison. Preserve
...
behavior, read `strawberry/set-up-shared-team-workflow`. Start from that accepted individual
