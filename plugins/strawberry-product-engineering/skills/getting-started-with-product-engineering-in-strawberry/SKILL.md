...
and separate observed facts from likely causes. Read `strawberry/investigate-fix-app-issues` for
...
login. Read `strawberry/extract-web-data` to agree the fields, source boundaries, row count, depth,
...
gets lost. Read `strawberry/transcribe-meetings-follow-up` to preserve those details in the agreed
...
current sources. Read `strawberry/research-companies-create-decks` when the result should be a
sourced visual story, or `strawberry/run-competitor-analysis` for a reviewable market comparison.
